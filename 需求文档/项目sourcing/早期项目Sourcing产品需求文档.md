...
### 3.5～3.7 竞品分析与投前项目（已迁出）

> **产品真源**：[竞品分析应用说明.md](../竞品分析/竞品分析应用说明.md)（菜单、三源召回、权限、接口、数据模型与验收口径）。下文仅保留与**项目挖掘**的边界说明。

| 主题 | 说明 |
|------|------|
| 独立应用 | `applications.id=2026051712000000001`，应用名 **竞品分析**；路由 `/dashboard/competitor-analysis/*`，API `/api/competitor-analysis/*` |
| 项目挖掘保留 | **融资与市场概览**、**融资事件**、**赛道配置**；融资信息源在「系统配置 > 融资信息源配置」维护 |
| 跨应用读取 | 竞品任务可读 `sourcing_financing_event`（须用户有**项目挖掘**权限且 `competitor_recall_source_config.enable_financing_event=1`） |
| 三源召回 | 底层项目 / 融资事件 / 联网发现；管理员在「系统配置 > 竞品三源召回」维护 |
| 被投/投前/底层项目 | `data_app_id` 已迁至竞品分析应用；企业管理筛选 `data_app_name=竞品分析` |
| AI 配置 | `ai_model_config.application_type=competitor_analysis`，`usage_type=competitor_match`；**不回退**项目挖掘分析模型 |
