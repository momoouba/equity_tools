...
> - **2026-5-14（竞品分析 Tab1 / 投前项目 Tab2 · 规划）**：项目挖掘新增 **竞品分析**（Tab1：被投企业 × 竞品列表）与 **投前项目**（Tab2）；多数据源 **Badge**；列表默认排序与 **被投企业列表多选批量竞品分析**（不支持对全表一次跑、重跑确认）等，见 **§3.5～§3.7**。**路由名**见 **§8.5、§8.6**；**企查查介绍清洗**、**以产品介绍(AI)+业务标签为主信号**、**信息不足时补录** 等见 **§3.5.3、§3.5.4**。（**评分、落库门槛、三源算法** 以 **2026-5-16** 修订为准，见 **§3.5.2、§3.5.3.1、§3.7.1**。）
> - **2026-5-16（竞品分析 · 三源匹配与置信度）**：废除「仅单次 LLM、&lt;90 不落库」；改为 **底层项目 + `sourcing_financing_event` + AI 联网** 三源；**阿里云** 关键词提取 / 文本相似度 /（可选）文本分类 **替代自托管 BERT**；**综合置信度 = 内部双源 60% + AI 源 40%**，**&lt;60 不落库**；等级 **S/A/B/C**；底层池 **近 3 年** `ipo_project`（项目挖掘应用）；融资侧 **轮次不参与轮次级差打分**，仅用 **最近 `event_date`** 选取代表事件；标签字段 **统一解析为中文字符串数组**；AI 源 **联网 JSON + 规则库 + 二次 LLM** 排上下游；**非竞品反馈全局降权** + **每周批量重算边界由配置与审批控制**；**S/A 依赖上线后持续人工抽检**；**投前 Tab2 同算法阈值**（数据来自投前录入）；**权限与创建人/会员角色对齐 §3.2**。
> - **2026-5-16（竞品分析 · 大模型编排）**：新增 **§3.5.10**（阿里云 **API Key / glm-5.1** 调用链、入参 JSON、多轮召回与收敛）、**§3.5.11**（评审待澄清）、**§12.13**（竞品专用 `prompt_type` 与默认提示词骨架）；与现有 **`ai_model_config` + DashScope OpenAI 兼容端点** 对齐。
> - **2026-5-16（竞品关系表 · 投前共用）**：**投前竞品与被投竞品均写入同一张 `sourcing_competitor_relation`**；通过 **`subject_type` + `pre_investment_project_id` / `pre_investment_run_id`** 等字段区分主体；见 **§3.5.7.2**。
...
- **综合置信度（0～100）**：由 **§3.5.3.1** 三源匹配产出 **内部数据源得分**（底层项目分支 + 融资事件分支的融合，研发须在契约中写清单侧仅有其一时的归一方式）与 **AI 检索得分**，再按 **加权平均**：**综合置信度 = 内部数据源得分 ×0.6 + AI 检索得分 ×0.4**。
- **等级（展示）**：**S** 90–100、**A** 80–89、**B** 70–79、**C** 60–69；UI 须同时展示 **分数 + 等级**。
- **落库与列表**：**综合置信度 &lt;60** 的候选 **不落库**（不写竞品关系表/不写主表快照）、**不出现在 Tab1 列表与导出**；任务运行日志可记录低于阈值的抽样便于调参（不作为产品列表数据源）。
- **评分明细**：须在持久化结构中保存 **各维度得分**（如标签、行业、关键词、阿里云相似度、业务规则分等），竞品分析页 **支持用户展开查看评分依据**。
- **准确率与运营**：**S/A 级** 竞品要求上线后 **持续人工抽检**（台账、频次与样本量由运营与研发约定）；不以单次离线评测替代线上质检闭环。
- **「非竞品」反馈**：用户对某竞品主体反馈 **非竞品** 后，须 **全局降权** 该企业作为竞品候选（后续任意被投任务的召回/重算均生效，具体降权或黑名单策略由研发实现）。**每周** 是否自动批量重算受影响范围，须由 **产品配置 + 运维/管理员审批** 界定自动化边界，避免无人值守误伤。
- **阿里云能力（替代 BERT）**：**不自托管** `bert-base-chinese`。**（a）预处理**：调用 **关键词提取 API**，输入目标与竞品的 **产品简介、经 §3.5.3 清洗后的企查查介绍**，输出双方 **核心关键词**，用于补充标签维、缓解「同义不同词」。**（b）相似度**：调用 **文本相似度 API**，以 **产品简介** 为主文本、**企查查介绍** 为辅助文本，输出 **0～100** 语义相似得分，**直接替代**原「向量 + 余弦」方案。**（c）可选**：**文本分类 API** 基于介绍输出行业路径（如 `先进制造>半导体设备`），用于 **交叉验证** 表内 **行业 (L1/L2)**，减轻人工标注错误带来的错配。
- **AI 检索（第三源）**：**复用** 本文 **模型原生联网 + 结构化 JSON**（见 §12 提示词与 `max_tokens`/重试/校验约定）；对候选须验证 **与目标行业一致**、**核心业务重叠度 ≥60%**，并 **排除上下游**：采用 **规则库 + 二次 LLM 判定**（与首次联网归纳分离）；命中后 **Badge「AI 检索」**。
- **投前 Tab2**：除 **企业维度数据来自投前表用户录入/企查查回填** 外，**与 Tab1 使用同一套三源算法、阈值、融合与展示规则**（§3.6.4）。
- **权限**：竞品分析 **发起、查看、导出、反馈** 与 **§3.2** 对齐——**项目挖掘应用可见**；**创建人** 可操作本人创建的被投/投前主体，**普通会员** 按权限表与 **手动 AI** 竞品入口一致；**admin** 全局；**不再** 以「仅 admin MVP」为终态。

#### 3.5.3 数据源一：融资事件 + 底层项目（结构化池）

- **融资事件（`sourcing_financing_event`）**：时间窗 **默认可配置，建议与底层池对齐为近 3 年**（起止闭区间；**上限**如 5 年由服务端校验防滥用）。仅纳入具备 **可参与匹配** 的 AI/文本字段的行（如 **产品简介(AI)**、**企业标签(AI)** 等，具体非空规则与现网字段对齐）。**同一竞品企业在多笔融资时**：**融资轮次字段不参与「轮次级差」打分**（全国轮次无统一标准，本阶段不建枚举比对）；轮次/轮次描述 **仅用于选取 `event_date` 最近的一条**（或研发明确的多条折叠策略）作为该企业在 **融资维度** 上的代表行。
- **底层项目（`ipo_project`）**：仅 **`data_app_id` = 项目挖掘** 的行；**仅纳入近 3 年** 数据（以 **`f_update_time` / `updated_at` / 创建时间`** 之一为业务口径，**字段名与闭区间由研发在详细设计中写死**）。与融资池 **各自先去重再参与匹配**；去重键优先 **`unified_credit_code`**，无码则 **规范化企业全称** 等（与 §3.5.4 一致）。
- **标签 JSON 口径**：`sourcing_financing_event.ai_company_tags_json`、`ipo_project.ai_industry_tags_json`、`invested_enterprises.ai_industry_tags_json` 在竞品流程中 **统一解析为「中文字符串标签数组」**（示例：`["工业机器人","AMR","功能安全认证"]`）。若历史数据为 **§12.10 对象键（`product_category`/`track` 等）+ 数组值** 形态，服务端 **须扁平合并** 为同一标签集合后，再参与 **Jaccard** 等计算。
- **匹配流程**：**① 底层项目匹配 → ② 融资事件匹配 → ③ AI 联网补充**，再融合为 §3.5.2 的综合分；禁止「被投 × 全表无筛选」直连大模型逐对；须有 **召回上限、限流、单企业耗时上限**（与 **§3.5.3.1** 一致）。
...
#### 3.5.3.1 多源匹配顺序、维度权重与入池门槛

- **执行顺序**：**① 底层项目表** → **② 融资事件表 `sourcing_financing_event`** → **③ AI 联网检索**；①②的匹配分经融合得到 **内部数据源得分**（0～100；仅命中一侧时的归一、同一主体多源合并取 max 或加权，由 **接口契约写死**），再与 **③ AI 检索得分** 按 §3.5.2 计算 **综合置信度**。
- **① 底层项目匹配**（候选为近 3 年、项目挖掘 `ipo_project`）：从目标企业提取 **核心标签、产品关键词**（含补录与 `ai_industry_tags_*`；文本侧辅以阿里云 **关键词提取 API** 对齐同义表达），与候选的 **企业标签(AI)、产品简介(AI)** 等比对。**维度权重**：标签（含 **Jaccard**）**35%** + **阿里云文本相似度 API**（产品简介为主、企查查介绍为辅）**40%** + 关键词 API 匹配 **15%** + **业务规则**（如子基金关联、重叠信号等）**10%**。**入「内部候选池」门槛**：本分支对某一竞品的 **加权得分 ≥70**。
- **② 融资事件匹配**（代表行取 **最近 `event_date`**，§3.5.3）：**行业 L1** 与目标侧 **须完全一致**；**行业 L2** 相似度 **≥80%**（字符串/层级算法与表字段映射见详细设计）。**维度权重**：行业层级 **35%** + **融资阶段占位 20%**（**不比轮次级差**，可用最新事件可解析度、`event_date` 新鲜度等规则占满该权重）+ 企业标签 **20%** + **产品侧文本相似度 API** **25%**。**入内部候选池门槛**：本分支 **加权得分 ≥60**。
- **③ AI 联网检索**：见 **§3.5.2**（联网 JSON、行业一致、核心重叠 ≥60%、规则库 + **二次 LLM** 排除上下游）；产出 **AI 检索得分**（0～100）。
- **动态权重（可选 · 阶段二）**：对技术密集型等行业，可在配置中 **提高标签/关键词维度占比**，默认以本节静态权重为准。
...
- **竞品列表字段**（折叠内明细）：**项目简称**、**被投企业全称**（竞品侧名称）、**统一社会信用代码**、**数据源**（Badge 集合语义）、**综合置信度**（0～100）、**等级（S/A/B/C）**、**评分明细入口**（展开各维度分）、**产品简介**、**企业标签**、**企业介绍**；**增加「相关子基金」**：自 **底层项目** 来源命中时，若有 **归属子基金/SPV** 等字段须写入；融资事件来源按标准表可映射则填，否则空。列表须支持 **按等级/分数筛选**、**导出**、**「非竞品」反馈**（触发 §3.5.2 全局降权规则）。

#### 3.5.5 数据源三：AI 搜索（不覆盖正文）

- 将 **被投企业侧** 的 **项目简称、企业名称、统一社会信用代码** 及经清洗的业务摘要，发给模型按独立提示词 **联网** 生成 **竞品候选列表**（**结构化 JSON**）；**数据源标记为「AI 搜索」**。须满足 **§3.5.2** 的二次校验（规则库 + **二次 LLM**），避免上下游误入选。
...
- **「竞品关系」统一表 `sourcing_competitor_relation`**：**被投 Tab1** 与 **投前 Tab2** 的竞品明细 **均写入此表**（不另建投前专用 relation 表）。除竞品侧字段外，须用 **主体维度字段** 区分「这条关系属于哪一家被分析企业」：
  - **被投**：`subject_type = invested_enterprise`，填 **`invested_enterprise_id`**，任务 **`run_id` → `sourcing_competitor_run`**。
  - **投前**：`subject_type = pre_investment_project`，填 **`pre_investment_project_id`**，任务 **`pre_investment_run_id` → `sourcing_pre_investment_competitor_run`**；**`invested_enterprise_id` 为空**。
- **竞品侧字段（两主体共用）**：**竞品 `unified_credit_code`（有码时主键归并）**、**无码时 `competitor_weak_key`（§3.5.3）**、**综合置信度**（0～100；可与 **`relevance_score`** 同义）、**等级（S/A/B/C）**、**评分明细 JSON**、**data_sources_json**、**related_sub_fund**、**financing_amount_text**、溯源 id（融资事件 id、`ipo_project.f_id` 等可空）、**用户反馈**（非竞品 → §3.5.2 全局降权）。DDL 与约束见 **§3.5.7.2**。
- **竞品主数据复用一张表（阶段二，可选）**：**有 `unified_credit_code` 时** 以之为业务主键存竞品企业维度信息（名称、简介、标签、企查查介绍等快照），多被投引用同一竞品时 **不重复存全文**（关系表引用主表）；**无码竞品** 可先以 **弱键 + 快照行** 落库，补全信用代码后 **迁移归并**（§3.5.3）。**当前库表（MVP）尚未单独建竞品主数据表**，展示字段先落在 **`sourcing_competitor_relation`**（及后续迁移增补列，见 **§3.5.7.1**）。

##### 3.5.7.1 表职责与前端读数（与 `db.js` 对齐）

| 表名 | 是否「竞品清单」 | 用途 | 前端 / 接口 |
|------|------------------|------|-------------|
| **`sourcing_competitor_relation`** | **是（唯一竞品明细表）** | **被投 + 投前** 共用：按 `subject_type` 区分主体，一行一条竞品；Tab1 / 投前页展开列表、导出均读此表 | 被投：`?invested_enterprise_id=`；投前：`?pre_investment_project_id=`（待实现） |
| **`sourcing_competitor_run`** | 否 | **被投** 单次分析任务状态 | `POST .../invested-enterprises/:id/competitor-analysis-run` |
| **`sourcing_competitor_run_step_log`** | 否 | **每步模型调用** 留痕（§3.5.11 #8 已闭合：独立子表） | 管理端 / 抽检，前端默认不展示 |
| **`competitor_match_supplement`** | 否 | 分析前 **用户补录标签 / 自由文本 / AI 抽标签结果** | 补录弹窗保存；就绪校验读取 |
| **`sourcing_pre_investment_competitor_run`** | 否 | **投前** 单次分析任务状态（与 `sourcing_competitor_run` 分表，但竞品结果仍写 **`sourcing_competitor_relation`**） | 投前 `POST .../pre-investment-projects/:id/competitor-analysis-run` |
| **`invested_enterprises`** | 否（主行） | 竞品分析页 **被投主行**（折叠标题：基金、退出状态、产品介绍摘要等） | `GET /api/enterprises?data_app_name=项目挖掘` |
| **`pre_investment_project`** | 否（投前主行） | 投前列表主数据 | 投前页 `GET .../pre-investment-projects` |

**结论（产品口径）**：**竞品明细只有一张表 `sourcing_competitor_relation`**；**被投主行** = `invested_enterprises`，**投前主行** = `pre_investment_project`；**任务表** 被投/投前各一张，通过 relation 上的 **`run_id` / `pre_investment_run_id`** 关联。现网 MVP 的 `invested_enterprise_id NOT NULL` 须 **迁移** 为 §3.5.7.2 可空 + 约束。

##### 3.5.7.2 `sourcing_competitor_relation` 扩展字段（投前共用 · 2026-5-16 已闭合）

在既有竞品列基础上 **迁移增补**（`db.js` 实现时按序执行）：

| 字段 | 类型 | 说明 |
|------|------|------|
| **`subject_type`** | `VARCHAR(32) NOT NULL` | **`invested_enterprise`** \| **`pre_investment_project`** |
| **`invested_enterprise_id`** | `VARCHAR(19) NULL` | 被投主体 id；`subject_type=invested_enterprise` 时 **必填** |
| **`pre_investment_project_id`** | `VARCHAR(19) NULL` | 投前主体 id；`subject_type=pre_investment_project` 时 **必填** |
| **`run_id`** | `VARCHAR(19) NULL` | 关联 **`sourcing_competitor_run.id`**（被投任务） |
| **`pre_investment_run_id`** | `VARCHAR(19) NULL` | 关联 **`sourcing_pre_investment_competitor_run.id`**（投前任务） |
| **`subject_display_name`** | `VARCHAR(255) NULL` | 冗余：分析当时的主体企业全称/简称，便于导出与审计 |
| （规划列）**`confidence_grade`** | `CHAR(1) NULL` | S/A/B/C |
| （规划列）**`score_breakdown_json`** | `JSON NULL` | 各维度及三源分项 |

**约束（逻辑）**：

- `subject_type = invested_enterprise` → `invested_enterprise_id IS NOT NULL` 且 `pre_investment_project_id IS NULL` 且 `pre_investment_run_id IS NULL`（`run_id` 建议非空）。  
- `subject_type = pre_investment_project` → `pre_investment_project_id IS NOT NULL` 且 `invested_enterprise_id IS NULL`（`pre_investment_run_id` 建议非空）。  
- **禁止** 同一行同时填被投 id 与投前 id。  
- 索引：`idx_rel_subject_ie (subject_type, invested_enterprise_id, delete_mark)`、`idx_rel_subject_pip (subject_type, pre_investment_project_id, delete_mark)`。  
- 外键：`pre_investment_project_id` → `pre_investment_project(id)`；`pre_investment_run_id` → `sourcing_pre_investment_competitor_run(id)`。

**查询约定**：

- 竞品分析 Tab：仅 `subject_type=invested_enterprise` + `invested_enterprise_id=?`。  
- 投前 Tab 展开竞品：仅 `subject_type=pre_investment_project` + `pre_investment_project_id=?`。  
- **不得** 用 `invested_enterprise_id` 查投前数据。

**历史数据**：迁移前已写入行默认 `subject_type = invested_enterprise`，`pre_investment_*` 为空。
...
#### 3.5.10 大模型编排（阿里云 API Key · 计划模型 **glm-5.1**）

> **状态**：产品真源（2026-5-16）。实现须复用系统 **`ai_model_config` / `ai_prompt_config`**，**禁止**在业务代码硬编码 Key；HTTP 走 **DashScope OpenAI 兼容** `.../compatible-mode/v1/chat/completions`（与现网 `dashScopeOpenAICompat`、`financingAiEnrichService` 一致）。

##### 3.5.10.1 配置与鉴权

| 配置项 | 约定 |
|--------|------|
| 模型表 | **`ai_model_config`**：`api_key`（阿里云 DashScope Key）、`api_endpoint`（兼容模式 URL）、`model_name` = **`glm-5.1`**（或控制台实际 model id，须与 `base_dictionary.ai_model_alibaba` 字典项一致） |
| 应用类型 | 建议新增 **`application_type = project_sourcing_competitor`**、`usage_type = competitor_match`（与融资增强的 `project_sourcing_analysis` **分配置**，避免误用联网增强提示词） |
| 提示词 | **`ai_prompt_config`**，按 **`prompt_type`** 分步绑定（见 **§12.13**）；支持在「系统设置 → AI 模型配置」热更新 |
| 联网 | **第三源「AI 检索」** 与 **补录自由文本抽标签** 须 **开启模型原生联网**（与 §12.10 一致）；**关键词/相似度/校验** 步骤默认 **不联网**（仅结构化 JSON） |
| 日志 | 每次模型调用写入 **`sourcing_competitor_run`** 关联的 **步骤日志**（JSON：`step`、`prompt_type`、`tokens`、耗时、原始响应摘要、解析成败）；敏感信息不落库 |

##### 3.5.10.2 单次任务状态机（`sourcing_competitor_run`）

```
queued → running → { success | partial | failed }
         ↑___________|  （用户/系统触发「继续分析」时从 partial 续跑）
```

- **`success`**：至少 1 条竞品 **综合置信度 ≥60** 已落库，且任务未处于「强制扩召回」状态。  
- **`partial`**：流程跑完但 **落库条数为 0**，或 **仅 C 级** 且运营配置要求「至少 1 条 B 级以上」；允许 **自动扩召回** 或用户 **继续分析**。  
- **`failed`**：就绪校验失败、连续 JSON 解析失败、或超过 **最大轮次/超时**。

##### 3.5.10.3 编排总览（与 §3.5.3.1 对齐）

```mermaid
flowchart TD
  A[就绪校验 + 组装 target_profile] --> B[源1: 底层项目 SQL 召回 + 规则分]
  B --> C[源2: 融资事件 SQL 召回 + 规则分]
  C --> D[合并内部候选池 Top-N]
  D --> E[批量 LLM: 关键词 + 相似度 + 可选行业分类]
  E --> F[计算 internal_score]
  F --> G[源3: 联网 LLM 竞品 JSON 召回]
  G --> H[规则库 + 二次 LLM 校验]
  H --> I[计算 ai_score + 综合置信度]
  I --> J{≥60 且通过校验?}
  J -->|是| K[落库 sourcing_competitor_relation]
  J -->|否| L[仅写运行日志]
  K --> M{结果满意?}
  L --> M
  M -->|自动/用户继续| N[扩召回 round+1]
  N --> B
  M -->|结束| O[返回竞品清单]
```

**硬约束**：**禁止**「目标企业 × 全库融资/底层」逐对直连大模型；**先 SQL/规则缩候选**，再对 **Top-N（建议 N≤80/企业/轮）** 调模型。

##### 3.5.10.4 组装给模型的「目标企业画像」`target_profile`

服务端在 **就绪校验通过** 后构造 **固定 JSON**（写入当次 `run` 上下文，各步提示词 **只读引用**）：
...
```json
{
  ...
  "unified_credit_code": "…",
  "product_intro": "产品介绍(AI) 全文",
  "qcc_intro_effective": "清洗后有效企查查介绍，无则空串",
  "tags_flat": ["工业机器人", "AMR"],
  "supplement_tags": ["用户补录标签"],
  "industry_l1": "来自被投或 AI 分类，可空",
  "industry_l2": "可空",
  "search_keywords": ["用于第三源检索的组合词"]
}
```

- **`search_keywords`**：规则生成 + 可选 **轻量 LLM**（`prompt_type = competitor_keyword_expand`，**不联网**）产出 3～8 条，例如 `[企业简称]+竞品+[核心产品]`、`[核心技术]+行业+主要厂商`。  
- 文本字段 **单段上限**（如 product_intro 4000 字）由服务端截断并记入日志。

##### 3.5.10.5 分步调用：输入 / 输出 / 用途

| 步骤 | prompt_type（§12.13） | 联网 | 输入要点 | 输出 JSON（仅 JSON，无 Markdown 围栏） | 用途 |
|------|------------------------|------|----------|----------------------------------------|------|
| S0 | `competitor_supplement_extract_tags` | 否 | 用户自由文本 | `{ "tags": [], "short_summary": "" }` | 补录弹窗（已有能力，竞品 run 前） |
| S1 | `competitor_pair_keywords` | 否 | `target_profile` + **单条** `candidate`（名称、简介、企查查、tags） | `{ "target_keywords": [], "candidate_keywords": [] }` | 底层/融资 **关键词维 15%** |
| S2 | `competitor_pair_similarity` | 否 | 双方 `product_intro`（主）、`qcc_intro_effective`（辅） | `{ "similarity_score": 0-100, "rationale": "一句" }` | 底层 **40%**、融资 **25%** 产品维 |
| S3 | `competitor_industry_classify` | 否 | 企业介绍文本 | `{ "industry_path": "L1>L2", "confidence": 0-100 }` | **可选**；与表内 L1/L2 **交叉验证** |
| S4 | `competitor_web_discover` | **是** | `target_profile` + `search_keywords` + `round` + `exclude_names[]` | 见 **§3.5.10.6** | 第三源候选列表 |
| S5 | `competitor_validate_candidate` | 否 | `target_profile` + 单条候选（含来源） | 见 **§3.5.10.7** | 行业一致、重叠度、是否上下游 |
| S6 | `competitor_refine_query` | 否 | 上一轮摘要 + `user_hints` + 已落库/已排除列表 | `{ "extra_keywords": [], "relax_internal_threshold": true/false }` | **继续分析** 前扩召回策略 |

**批量策略**：S1/S2 对 **同一 run 内候选** 按 **每批 5～10 家** 调用（或并行度受限的异步池），失败 **单条重试 2 次**，仍失败则该候选 **仅保留规则分**、不阻断整单。

**无会话上下文（强制，2026-5-16 已闭合）**：S0～S6 **每一步均为独立 HTTP 请求**——仅携带 **当步 System/User 提示词 + 本步所需 JSON 入参**；**禁止** multi-turn 对话、**禁止**把上一轮模型原文/assistant 历史拼进下一轮（避免上下文污染、分数漂移、竞品名单串台）。步骤间只通过 **服务端 `target_profile`、run 状态、数据库** 传递结构化结果，不通过模型记忆传递。

##### 3.5.10.6 第三源联网召回（S4）输出契约
...
```json
{
  ...
  "search_notes": "检索说明，可选"
}
```

- 模型须 **自带联网**；服务端 **不** 用 AI 正文覆盖内部源已落库的简介/标签（§3.5.5）。  
- 单轮 **`candidates` 上限 30**；与内部池 **按信用代码/规范化全称去重** 后进入 S5。

##### 3.5.10.7 二次校验（S5）输出契约
...
- **规则库（先于或并行 LLM）**：全局 **非竞品黑名单/降权表**、关键词「上游供应商/下游客户」、与 `target_profile.industry_l1` **字符串不等** 则 `industry_match=false`。  
- **`is_competitor=false` 或 `is_upstream_downstream=true`**：该候选 **不进入落库**，可记入 run 日志。  
- **`validated_score`** 映射为 **AI 源得分 `ai_score`（0～100）**；与 **§3.5.2** 融合。

##### 3.5.10.8 内部分与综合分（服务端确定性计算）

1. **底层分支分** `score_ipo`：按 §3.5.3.1 权重，其中「相似度」取 S2 的 `similarity_score`，「关键词」取 S1 集合 **Jaccard 或命中率**；**≥70** 进入内部池。  
2. **融资分支分** `score_fin`：行业 L1/L2 规则 + S2/S3 + 标签 Jaccard；**≥60** 进入内部池。  
3. **内部数据源得分** `internal_score`：同一竞品主体多源命中时 **`max(score_ipo, score_fin)`**（详细设计可改为加权，须在接口文档写死）。  
4. **综合置信度** = `internal_score * 0.6 + ai_score * 0.4`（仅第三源命中时 `internal_score` 可为 0，此时 **仅 AI 分不单独落库** 除非产品另定——**默认：internal 与 ai 至少一侧 ≥60 等价门槛** 才落库，见 §3.5.11 待澄清 #1）。  
5. **&lt;60** 丢弃；**≥60** 写 `sourcing_competitor_relation` + `score_breakdown_json`。

##### 3.5.10.9 结果不满意时：如何继续执行直至「合理」

**合理结果的默认定义（产品）**：单次 run **落库竞品 ≥3 家且至少 1 家等级 ≥B（70）**；或用户 **手动确认完成**。低于该标准进入 **扩召回**。

| 触发 | 行为 | 上限 |
|------|------|------|
| **自动扩召回** | `round`+1：① 内部池门槛 **-5**（底层 65、融资 55，下限 50）；② S4 追加 **`extra_keywords`**（S6）；③ `exclude_names` = 已校验失败名单；④ 融资时间窗 **+1 年**（最多扩至 5 年） | **每任务自动最多 2 轮**（共 3 pass） |
| **用户「继续分析」** | 携带 `prior_run_id`、可选 **`user_hints`**（如「补充海外竞品」「排除某某」）；执行 S6 后走 **新一轮 S4～S5**，**不删除** 已确认落库行，除非用户勾选 **「重跑并覆盖」** | 与 **重跑确认**（§3.5.9）互斥逻辑由前端二选一 |
| **用户「非竞品」反馈** | 写入全局降权表；**不自动** 在本 run 内删除已落库行（可选产品：立即隐藏） | — |
| **JSON 解析失败** | 同 prompt **重试 2 次**；仍失败则该步骤 `failed`，整单 **partial**，提示管理员查看日志 | — |
| **仍不满足** | `status=partial`，前端展示「已分析 N 家候选，落库 M 家；可继续分析或调整补录标签后重试」 | — |

**收敛**：达到「合理」或触及轮次上限后 **停止扩召回**；最终以 **综合置信度降序** 展示，用户可 **筛选/导出/反馈**。

##### 3.5.10.10 与现有代码的落地锚点（研发）

- 鉴权与 HTTP：**`newsAnalysis.callAIModel(prompt, config)`** + **`normalizeDashScopeChatEndpoint`**。  
- 配置读取：参照 **`projectMiningAi.getActiveProjectMiningModelConfig`**，改为竞品专用 `application_type`。  
- 异步任务：**`sourcing_competitor_run.status`** 由 stub 改为上述状态机；关系写入 **`sourcing_competitor_relation`**。  
- 投前：**`sourcing_pre_investment_competitor_run`** 记录任务；竞品行写入 **`sourcing_competitor_relation`**（`subject_type=pre_investment_project`，§3.5.7.2）。

#### 3.5.11 大模型编排 — 已闭合决策（2026-5-16 评审）

| # | 决策 |
|---|------|
| 1 | **仅 AI 源命中、内部源未命中** → **不允许落库**；须至少命中 **底层或融资** 之一且该侧过线，AI 用于加分/补源/Badge。 |
| 2 | **`glm-5.1` 支持原生联网**；第三源 **S4** 使用联网；若环境异常再降级 `partial`（运维监控）。 |
| 3 | **阶段一统一使用同一 `ai_model_config`（glm-5.1）+ 不同 `prompt_type`**，**不**接独立 NLP API；**每步独立请求、不关联多轮上下文**（见 §3.5.10.5「无会话上下文」）。 |
| 4 | 每企业每轮对候选调 S1/S2 上限 **Top-N=80**，超出 **仅规则分**。 |
| 5 | 自动扩召回默认条件：**落库 ≥3 家且至少 1 家等级 ≥B（70）**；可在系统配置关闭。 |
| 6 | **继续分析** 默认 **保留** 已落库竞品；覆盖式重跑仅 **§3.5.9** 二次确认后整批作废重算。 |
| 7 | 竞品编排 **单独一条** `ai_model_config`（`application_type=project_sourcing_competitor`），与融资增强 **不共用** 同一条配置记录。 |
| 8 | 模型步骤日志使用独立表 **`sourcing_competitor_run_step_log`**（关联 `run_id`），**不** 仅写入 `message` 长文本。 |
...
- 提供按钮 **「竞品分析」**：执行 **与 Tab1 相同的三源匹配、阿里云 API、综合置信度融合与落库规则**（**§3.5.2、§3.5.3、§3.5.3.1、§3.5.10**）；**唯一差异**为 `target_profile` 取自 **`pre_investment_project`**（用户录入/企查查/AI 回填）。
- **落库**：竞品明细写入 **`sourcing_competitor_relation`**，`subject_type=pre_investment_project`，填 **`pre_investment_project_id`**、**`pre_investment_run_id`**；**不** 写入 `invested_enterprise_id`（§3.5.7.2）。
...
- **重复投前**：与 **被投企业（项目挖掘）**、**竞品主数据/关系** 判重；若已存在则 **不入库** 并 **明确提示** 去向（例：「已在被投企业，请到被投企业查看」或「已在竞品分析」）。**跨主体说明**：同一 **竞品企业**（信用代码相同）可同时出现在 **被投 A 的 relation 行** 与 **投前 B 的 relation 行**（主体不同、各行独立）；判重指 **投前项目本身** 是否已升级为被投，而非禁止竞品行重复。

---

### 3.7 竞品与投前 — 需求评审纪要（2026-5-14，**2026-5-16** 竞品算法修订同步）

#### 3.7.1 已闭合的产品决策

| 项 | 结论 |
|----|------|
| 低分不落库 | **综合置信度 &lt;60 不写库**，列表与导出均不出现。（**2026-5-16** 修订：废止原「&lt;90」单品规，见 **§3.5.2**。） |
| 多源命中展示 | **Badge** 多枚；正文以非 AI 搜索源为准，AI 搜索仅补 **来源标签** 与 **去重主键对齐**。 |
| 默认排序 | **被投企业名称升序**；支持切换 **最近竞品更新时间**、**当前被投下竞品综合置信度最大值**（降序）。 |
...
| 语义与打分 | **阿里云** 关键词提取 + 文本相似度 +（可选）文本分类；**不自托管 BERT**；三源与融合见 **§3.5.2、§3.5.3.1**。 |
| 权限 | **创建人 + 普通会员 + admin** 与 **§3.2、§3.5.2** 对齐（**2026-5-16**：不再以「仅 admin MVP」为终态）。 |
| S/A 质检 | **上线后持续人工抽检**（台账与频次运营约定）。 |
| 非竞品反馈 | **全局降权**；**每周** 自动批量重算须 **配置 + 审批** 界定边界（§3.5.2）。 |
| 模型调用 | **glm-5.1** 分步；**每步独立请求、不携带多轮上下文**（§3.5.10.5、§3.5.11 #3）。 |
| 竞品读表 | 竞品明细 **统一 `sourcing_competitor_relation`**；被投/投前靠 **`subject_type` + 对应主体 id** 查询（§3.5.7.2）。 |
...
| 权限 | 与 **§3.7.1** 已闭合结论一致；实现时须与 **§3.2**、路由守卫、权限表 **回归对齐**，避免代码仍写死仅 admin。 |
...
- 表 DDL、API 清单、竞品 **步骤日志表**、与 `invested_enterprise_ai_enrich_log` 是否共用任务日志，在 **详细设计 / db.js** 中展开；本文 **§3.5～§3.6**（含 **§3.5.10～§3.5.11**）为产品真源。
...
16. **（竞品 · 2026-5-16）**全量流水线须符合 **§3.5.2、§3.5.3、§3.5.3.1、§3.5.10、§3.7.1**：三源顺序、**综合置信度 &lt;60 不落库**、**S/A/B/C** 展示、**glm-5.1（或配置的竞品模型）** 分步调用（§12.13）、**内部 60% + AI 40%** 融合、**评分明细**可展开、**自动/用户继续分析**扩召回、列表 **筛选/导出/非竞品反馈**；**投前 Tab2** 同算法；**权限**与创建人/会员一致；**S/A 人工抽检**台账可导出。
...
### 12.13 竞品分析专用提示词类型（**2026-5-16**，编排见 **§3.5.10**）

> **模型**：计划在 **`ai_model_config`** 中配置 **`model_name = glm-5.1`**（以 DashScope 控制台为准）、**`application_type = project_sourcing_competitor`**。  
> **调用**：`newsAnalysis.callAIModel` + OpenAI 兼容端点；**S4** 须模型支持 **原生联网**。

| `prompt_type` | 联网 | 占位符 / 入参 |
|---------------|------|----------------|
| `competitor_supplement_extract_tags` | 否 | `{{NARRATIVE}}` |
| `competitor_pair_keywords` | 否 | `{{TARGET_PROFILE_JSON}}`、`{{CANDIDATE_JSON}}` |
| `competitor_pair_similarity` | 否 | 同上 |
| `competitor_industry_classify` | 否 | `{{INTRO_TEXT}}` |
| `competitor_web_discover` | **是** | `{{TARGET_PROFILE_JSON}}`、`{{SEARCH_KEYWORDS_JSON}}`、`{{ROUND}}`、`{{EXCLUDE_NAMES_JSON}}` |
| `competitor_validate_candidate` | 否 | `{{TARGET_PROFILE_JSON}}`、`{{CANDIDATE_JSON}}` |
| `competitor_refine_query` | 否 | `{{PRIOR_RUN_SUMMARY}}`、`{{USER_HINTS}}`、`{{EXCLUDED_JSON}}` |

**`competitor_web_discover` — System 骨架（测试稿）**

```text
你是竞品研究助理。你必须使用联网搜索，根据目标企业画像与检索词组合，找出「提供同类产品或服务的公司」。
仅输出一个 JSON，禁止 Markdown 围栏与 JSON 外文字。
禁止把上游原材料供应商、下游渠道商、母公司集团当作竞品。
字段见 user 消息中的 schema；ai_relevance_score 为 0～100 的整数自评。
```

**`competitor_validate_candidate` — System 骨架（测试稿）**

```text
你是竞品校验助手。根据目标企业与候选企业信息，判断是否属于「直接竞品」。
仅输出 JSON：is_competitor、industry_match、core_overlap_percent(0-100)、is_upstream_downstream、validated_score(0-100)、reject_reason。
若为核心业务重叠不足 60%、或为明显上下游关系，则 is_competitor=false。
```

**验收（竞品编排）**：在测试环境用 **glm-5.1** 跑通 **S4→S5** 至少 3 家被投样本；JSON 可解析率 ≥95%；`sourcing_competitor_run` 步骤日志可回溯每次请求。
