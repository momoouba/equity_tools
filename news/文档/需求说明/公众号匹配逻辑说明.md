...
入库、同步、重新分析等路径查询 **`invested_enterprises`** 时，除退出状态等条件外，还须满足 **「新闻舆情」应用** 维度：优先 **`data_app_id`** 与 `applications` 中「新闻舆情」的 id 一致；**`data_app_id` 为空**的旧行按 **`data_app_name`** 回退为新闻舆情。统一 SQL 片段见 **`数据应用隔离与业务匹配说明.md`**（`investedEnterpriseNewsAppSql.js`）。
