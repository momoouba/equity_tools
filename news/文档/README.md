...
> **文档目录**：`AI模型相关/` · `docker部署相关/` · `部署相关/`（原 `deploy/` 运维备忘）· `错误问题/` · `集成功能/` · `接口相关/`（企查查、上海国际、国际集团接口）· `需求说明/` · 根目录部署总览（正常更新部署流程、运行指南等）。项目入口说明见 `news/README.md`。

**生产运维速链（`release/clean` 随 `news/` 发布）**

- [正常更新部署流程](./正常更新部署流程.md)
- [生产环境更新指南](./生产环境更新指南.md)
- [Docker 生产环境部署指南](./部署相关/Docker生产环境部署指南.md)
- [百度百科 CDP 与 Playwright 部署说明](./部署相关/百度百科CDP与Playwright部署说明.md)（百科批量查词生产推荐）
