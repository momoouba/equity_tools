-- b_project_a 累计弹窗
-- 仅母基金（INNER JOIN perf_fund）
-- F_Id=2026051508440200001
...
SELECT
    ...
    SUM(h.cost_1) AS project_amount,
    SUM(h.cost) AS total_amount,
    COUNT(DISTINCT CASE WHEN h.is_listed = 1 THEN h.company END) AS ipo_num,
    SUM(CASE WHEN h.is_listed = 1 THEN h.cost_1 ELSE 0 END) AS ipo_amount
FROM holdings h
