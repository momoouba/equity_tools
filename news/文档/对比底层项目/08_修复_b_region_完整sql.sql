-- b_region 当前地域：INNER JOIN 母基金 + 去掉 OR cost=0（避免负数）
-- F_Id=2026051508442000001
...
SELECT
    ...
    h.fund,
    '母基金' AS fund_type,
    DATE(pf.set_up_date) AS set_up_date,
    COUNT(*) AS t_num,
    SUM(h.net_cost) AS t_amount,
    COUNT(DISTINCT CASE WHEN c.region = '长三角' THEN h.company END) AS csj_num,
    SUM(CASE WHEN c.region = '长三角' THEN h.net_cost ELSE 0 END) AS csj_amount,
    SUM(CASE WHEN c.region = '长三角' THEN h.net_cost_1 ELSE 0 END) AS csj_amount_ct,
    COUNT(DISTINCT CASE WHEN c.region = '长三角' THEN h.company END) / NULLIF(COUNT(*), 0) AS csj_num_ratio,
    SUM(CASE WHEN c.region = '长三角' THEN h.net_cost ELSE 0 END) / NULLIF(SUM(h.net_cost), 0) AS csj_amount_ratio,
    COUNT(DISTINCT CASE WHEN c.shi = '上海市' THEN h.company END) AS sh_num,
    SUM(CASE WHEN c.shi = '上海市' THEN h.net_cost ELSE 0 END) AS sh_amount,
    SUM(CASE WHEN c.shi = '上海市' THEN h.net_cost_1 ELSE 0 END) AS sh_amount_ct,
    COUNT(DISTINCT CASE WHEN c.shi = '上海市' THEN h.company END) / NULLIF(COUNT(*), 0) AS sh_num_ratio,
    SUM(CASE WHEN c.shi = '上海市' THEN h.net_cost ELSE 0 END) / NULLIF(SUM(h.net_cost), 0) AS sh_amount_ratio,
    COUNT(DISTINCT CASE WHEN c.qu = '浦东新区' THEN h.company END) AS pd_num,
    SUM(CASE WHEN c.qu = '浦东新区' THEN h.net_cost ELSE 0 END) AS pd_amount,
    SUM(CASE WHEN c.qu = '浦东新区' THEN h.net_cost_1 ELSE 0 END) AS pd_amount_ct,
    COUNT(DISTINCT CASE WHEN c.qu = '浦东新区' THEN h.company END) / NULLIF(COUNT(*), 0) AS pd_num_ratio,
    SUM(CASE WHEN c.qu = '浦东新区' THEN h.net_cost ELSE 0 END) / NULLIF(SUM(h.net_cost), 0) AS pd_amount_ratio
FROM holdings h
LEFT JOIN perf_company c
    ON c.version = (SELECT current_version FROM current_version)
   AND c.F_DeleteMark = 0
   AND c.company = h.company
INNER JOIN perf_fund pf
    ON pf.version = (SELECT current_version FROM current_version)
   AND pf.F_DeleteMark = 0
...
WHERE h.fund IS NOT NULL AND TRIM(h.fund) <> ''
  AND h.fund <> '国方一期产品'
  AND (
    (h.fund_type = '子基金' AND h.net_cost > 0)
    OR (h.fund_type = '母基金' AND h.net_cost > 1000)
  )
GROUP BY h.fund, pf.set_up_date
ORDER BY pf.set_up_date ASC, h.fund
;
