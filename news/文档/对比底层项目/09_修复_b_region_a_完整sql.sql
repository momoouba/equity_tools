-- b_region_a 累计地域：INNER JOIN 母基金
-- F_Id=2026051508443500001
...
SELECT
    ...
    COUNT(*) AS t_num,
    SUM(h.cost) AS t_amount,
    COUNT(DISTINCT CASE WHEN c.region = '长三角' THEN h.company END) AS csj_num,
    SUM(CASE WHEN c.region = '长三角' THEN h.cost ELSE 0 END) AS csj_amount,
    SUM(CASE WHEN c.region = '长三角' THEN h.cost_1 ELSE 0 END) AS csj_amount_ct,
    COUNT(DISTINCT CASE WHEN c.region = '长三角' THEN h.company END) / NULLIF(COUNT(*), 0) AS csj_num_ratio,
    SUM(CASE WHEN c.region = '长三角' THEN h.cost ELSE 0 END) / NULLIF(SUM(h.cost), 0) AS csj_amount_ratio,
    COUNT(DISTINCT CASE WHEN c.shi = '上海市' THEN h.company END) AS sh_num,
    SUM(CASE WHEN c.shi = '上海市' THEN h.cost ELSE 0 END) AS sh_amount,
    SUM(CASE WHEN c.shi = '上海市' THEN h.cost_1 ELSE 0 END) AS sh_amount_ct,
    COUNT(DISTINCT CASE WHEN c.shi = '上海市' THEN h.company END) / NULLIF(COUNT(*), 0) AS sh_num_ratio,
    SUM(CASE WHEN c.shi = '上海市' THEN h.cost ELSE 0 END) / NULLIF(SUM(h.cost), 0) AS sh_amount_ratio,
    COUNT(DISTINCT CASE WHEN c.qu = '浦东新区' THEN h.company END) AS pd_num,
    SUM(CASE WHEN c.qu = '浦东新区' THEN h.cost ELSE 0 END) AS pd_amount,
    SUM(CASE WHEN c.qu = '浦东新区' THEN h.cost_1 ELSE 0 END) AS pd_amount_ct,
    COUNT(DISTINCT CASE WHEN c.qu = '浦东新区' THEN h.company END) / NULLIF(COUNT(*), 0) AS pd_num_ratio,
    SUM(CASE WHEN c.qu = '浦东新区' THEN h.cost ELSE 0 END) / NULLIF(SUM(h.cost), 0) AS pd_amount_ratio
FROM holdings h
