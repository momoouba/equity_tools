-- b_project 当前弹窗
-- 仅母基金（INNER JOIN perf_fund）；total_amount/ipo_amount 用 cost
-- F_Id=2026051508421400001
...
SELECT
    ...
    COUNT(DISTINCT CASE WHEN h.net_cost > 0 THEN h.company END) AS company_num,
    SUM(CASE WHEN h.net_cost > 0 THEN 1 ELSE 0 END) AS project_num,
    SUM(CASE WHEN h.net_cost > 0 THEN h.net_cost_1 ELSE 0 END) AS project_amount,
    SUM(CASE WHEN h.net_cost > 0 THEN h.cost ELSE 0 END) AS total_amount,
    SUM(CASE WHEN h.is_listed = 1 AND h.net_cost > 0 THEN 1 ELSE 0 END) AS ipo_num,
    SUM(CASE WHEN h.is_listed = 1 AND h.cost > 0 THEN h.cost ELSE 0 END) AS ipo_amount
FROM holdings h
