...
SELECT
    ...
    h.company,
    h.fund_type,
    ROUND(h.cost, 2) AS cost,
    ROUND(h.exit_cost, 2) AS exit_cost,
    ROUND(h.net_cost, 2) AS net_cost,
    ROUND(h.cost_1, 2) AS cost_1,
    ROUND(h.net_cost_1, 2) AS net_cost_1,
    CASE WHEN h.is_listed = 1 THEN '是' ELSE '否' END AS is_listed
FROM holdings h
WHERE h.fund IN ('国方一期', '国方二期')
  AND h.net_cost > 0
