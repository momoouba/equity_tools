...
### 步骤 3.5（可选）：安装 MarkItDown（BP 文件解析）

如果需要使用「投前竞品分析」中的 BP 上传功能，需要安装 MarkItDown（微软开源的文档转 Markdown 工具）。

**前置条件**：确保已安装 Python 3.10 及以上版本（运行 `python --version` 或 `python3 --version` 检查）。
...
> 如果不需要 BP 上传功能，可跳过此步骤，不影响其他功能正常使用。
...
### Q: BP 上传功能报错 "MarkItDown" 相关错误？
**A:** 这是 BP 文件解析所需的 Python 工具未安装。请确保已安装 Python 3.10+，然后运行：
```
pip install "markitdown[all]"
```
安装后重新上传 BP 文件即可。如果不需要 BP 上传功能，可以忽略此错误。
