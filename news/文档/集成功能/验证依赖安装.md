...
**上市进展 · 辅导备案**：抓取脚本为 `server/utils/listing/guidance_progress_fetch.py`，依赖同上；若仅 `pip install playwright` 而未执行 **`python3 -m playwright install chromium`**，辅导同步会回退为纯 HTTP 且无表头排序模拟。部署说明见 **`news/上市进展/辅导备案与Playwright部署说明.md`**。
