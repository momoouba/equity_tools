...
| exit_status | exit_status, exitStatus | 支持值：未退出、部分退出、完全退出、继续观察、已上市。注意：状态为"已上市"或"完全退出"的企业不会被企查查接口抓取新闻数据 |
...
4. **更新或插入**：
   - **如果统一社会信用代码一致**（且不为空）：
     - 更新现有记录的`project_abbreviation`（项目简称）、`enterprise_full_name`（被投企业全称）和`official_website`（企业官网）
     - 其他字段保持不变
   - **如果统一社会信用代码不一致或不存在**：
...
- 更新操作会更新项目简称、企业全称和企业官网，不会修改其他字段（如公众号ID、退出状态等）
