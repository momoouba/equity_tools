...
async function prepareInvestedEnterpriseAiJob({ enterpriseId, triggerType, triggeredByUserId, clientIp }) {
  ...
  const rows = await db.query(
    `SELECT id, enterprise_full_name, data_app_name, delete_mark
  ...
  if (String(rows[0].data_app_name || '') !== DATA_APP_PROJECT_SOURCING) {
    return { ok: false, code: 400, message: '仅支持项目挖掘应用下的被投企业' };
  }

  const dup = await db.query(
    `SELECT id, triggered_at FROM invested_enterprise_ai_enrich_log
     WHERE invested_enterprise_id = ? AND execution_status IN ('pending','running')
  ...
  const ins = await db.execute(
    `INSERT INTO invested_enterprise_ai_enrich_log
     (invested_enterprise_id, enterprise_full_name, trigger_type, triggered_by_user_id, client_ip, job_trace_id, execution_status, prompt_type, ai_enrich_version)
     VALUES (?, ?, ?, ?, ?, ?, 'pending', ?, ?)`,
    [
      id,
...
async function enqueueBatchInvestedEnterpriseAiEnrich({
  ...
  const rows = await db.query(
    `SELECT id, enterprise_full_name FROM invested_enterprises
     WHERE delete_mark = 0 AND data_app_name = ?
       AND DATE(created_at) >= ? AND DATE(created_at) <= ?
       ${failedClause}
     ORDER BY created_at DESC, id DESC`,
    [DATA_APP_PROJECT_SOURCING, df, dt]
  );
