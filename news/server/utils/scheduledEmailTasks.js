...
/** APItype 为上海国际/企查查且 account_name 为以下类型时：邮件不显示原文链接，且不参与标题/摘要语义相似度去重 */
const NO_LINK_NO_SEMANTIC_DEDUP_ACCOUNT_NAMES = ['裁判文书', '法院公告', '送达公告', '开庭公告', '立案信息', '破产重整', '被执行人', '失信被执行人', '限制高消费', '行政处罚', '终本案件'];

/**
 * 判断是否为“不显示原文链接、不参与语义去重”的类型：APItype 为上海国际/上海国际集团/企查查，且 account_name 在指定列表中。
 */
function isNoLinkNoSemanticDedupType(news) {
  ...
  return NO_LINK_NO_SEMANTIC_DEDUP_ACCOUNT_NAMES.includes(name);
}
...
async function deduplicateNewsBySemanticSimilarity(newsList, logTag = '[邮件发送]') {
  ...
  for (const [entName, group] of byEnterprise.entries()) {
    ...
    for (let i = 1; i < group.length; i++) {
      ...
      // 上海国际/企查查的裁判文书、法院公告等类型：不参与语义相似度去重，直接保留
      if (isNoLinkNoSemanticDedupType(candidate)) {
        kept.push(candidate);
        continue;
      }
      let isSimilar = false;
      for (const k of kept) {
        // 已保留项若是“不参与语义去重”类型，则不与之做相似度比较（不因它而丢弃当前条）
        if (isNoLinkNoSemanticDedupType(k)) continue;
