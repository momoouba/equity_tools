 ...
 *   cd /opt/newsapp/news && docker compose exec app node server/utils/listing/cleanupTraditionalDuplicates.js [--dry-run]
...
const possibleEnvPaths = [
  path.resolve(__dirname, '../../../.env'), // news/server/utils/listing -> news/.env
  ...
  if (h === '127.0.0.1' || h === 'localhost' || h === '::1') {
    console.warn(
      '[数据库] 从宿主机连本机 MySQL 时，易出现「仅有 news_app@\'%\'、无 news_app@\'localhost\'」导致 1045。\n' +
        '优先建议：在项目目录执行 docker compose exec <app服务名> node server/utils/listing/cleanupTraditionalDuplicates.js [--dry-run]'
    );
  }
