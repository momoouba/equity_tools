...
async function executeListingEmailDigest(recipient, options = {}) {
  ...
  if (includeOverseasFiling) {
    /** 境外备案：每日发送，查询当天定时同步入库的记录（F_CreatorTime = 今天），与每日抓取最近工作日的策略对齐 */
    // 独立查询境外上市备案数据，不依赖 ipo_project_progress 匹配表
    ipoOverseasFiling = await db.query(
  ...
  const part2OverseasFiling =
    ipoOverseasFiling.length === 0
      ? '<p style="margin:0 0 12px;color:#4e5969;">（昨日无新增境外发行备案记录）</p>'
      : `<table cellpadding="0" cellspacing="0" style="${tableBaseStyle}">
          <tr>
  ...
  if (includeOverseasFiling) {
    listingSectionHtml += `${renderSectionTitle('境内企业境外上市备案（昨日）', '#f7ba1e')}${part2OverseasFiling}`;
  }
  if (includeNewShareUpcoming) {
  ...
  const mailLogMeta = {
    ...
    stats: {
      ...
      overseasFilingCount: ipoOverseasFiling.length,
