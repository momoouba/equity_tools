...
/**
 * 硬删前快照本用户本应用下手动退出状态（不再观察/继续观察），供全量插入后回填。
...
async function snapshotManualExitStatuses(creatorUserId, dataAppName) {
  ...
  console.log(
    `[企业同步任务] ${dataAppName} 已快照手动退出状态 ${n} 条（不再观察/继续观察，供硬删后回填）`
  );
  return lookup;
}
...
async function executeSyncTask(
  ...
  if (manualExitRestored > 0) {
    console.log(`[企业同步任务] ${targetDataAppName} 已回填手动退出状态 ${manualExitRestored} 条（不再观察/继续观察）`);
  }
  ...
  const manualExitNote =
    manualExitRestored > 0
      ? `；已回填手动退出状态 ${manualExitRestored} 条（不再观察/继续观察）`
      : '';
