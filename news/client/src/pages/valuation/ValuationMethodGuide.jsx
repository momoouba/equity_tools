...
const STEPS = [
  ...
  },
  {
    id: 'fetch',
    title: '3. 系统取数',
    body: '每家可比股：利润表、资产负债表、现金流量表入库；估值倍数按交易日截面。方法配置里可填「市场法锚定日」（未填则自动填入案件创建日，走当天行情），计算时取该日或之前最近一个交易日的 PE/PS。三费、毛利率、营运天数都按公司、按年展示：三费是销售/管理/研发三行，营运是 DSO/DPO/DIO 三行。标的自身没有上市财报，利润表 / 资产负债 / 现金流需按万元手工录入（或导入）。',
  },
  {
    id: 'relative',
  ...
  },
  {
    id: 'market',
    title: '6. 市场法',
    body: '流通权益 = 倍数 × 基数；非流通 = 流通 × (1 − 市场法缺乏流动性折扣)，默认 30%，与并购 DCF 折扣分开。基数优先用锚定日所在年营收/净利润。P/S、P/E 各一行，只分低端和高端：低端 = POOL 中位数 − σ，高端 = POOL 中位数。计算后填进市场法倍数格；改数字会锁定覆盖 POOL，点「跟随 POOL」后只计算即恢复。亏损年仍算 P/E，并告警「利润为负，仅供参考」。行业法开启时，用申万三级 PE/PS 中位数替换个股 POOL，同样按锚定日及以前历史中位。',
  },
  {
    id: 'dcf',
    title: '7. DCF',
