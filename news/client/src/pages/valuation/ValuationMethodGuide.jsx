...
const STEPS = [
  ...
  },
  {
    id: 'comps',
    title: '2. 可比与采集',
    body: '可比公司先由 AI 提名境内上市同业并给出业务理由，再对照上市主档核实代码；同时按行业/赛道规则召回。也可从竞品分析最新成功 run、手工代码或 Excel 导入。名单可编辑删除；预览可见匹配理由。勾选且「入池」的股票进入 POOL。采集走东方财富境内行情（上交所 / 深交所 / 北交所 / 新三板），拉年报与历史 PE/PS。港股美股不入池。抓取失败时用库内已有财报，并在提示列说明。',
  },
  {
    id: 'fetch',
    title: '3. 系统取数',
  ...
  },
  {
    id: 'market',
    title: '6. 市场法',
    body: '流通权益 = 倍数 × 基数；非流通 = 流通 × (1 − 市场法缺乏流动性折扣)，默认 30%，与并购 DCF 折扣分开。基数优先用锚定日所在年营收/净利润。倍数默认取可比 POOL（低端 = −1σ，高端 = 中位），计算后填进市场法倍数格；改数字会锁定覆盖 POOL，点「跟随 POOL」后只计算即恢复。亏损年仍算 P/E，并告警「利润为负，仅供参考」。行业法开启时，用申万三级 PE/PS 中位数替换个股 POOL，同样按锚定日及以前历史中位。',
  },
  {
    id: 'dcf',
    title: '7. DCF',
