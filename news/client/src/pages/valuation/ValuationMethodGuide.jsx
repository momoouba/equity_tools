...
const STEPS = [
  ...
  },
  {
    id: 'target',
    title: '5. 标的三表',
    body: '利润表按万元。已实现最近一年（如 2025）是市场法基数：P/S 用营业收入，P/E 用净利润。预测年（如 2026E 起）进入 DCF；默认预测 5 年，可只填前两年，其后按收入增速与费用率外推。空白年份会跳过，不会当成 0。折现期数按锚定日对齐（例如锚定日在 2026 年时，2026E 为第 3 期），不因删掉更早的已实现年而重计为 t=1。资产负债表核心科目包括货币资金、应收/预付、存货、固资/在建/无形、短贷/长贷、应付/预收；净负债 = 短期借款 + 长期借款 − 货币资金。营运资本占用 = (应收票据+应收账款+预付款项+存货) − (应付票据+应付账款+预收款项)。',
  },
  {
    id: 'market',
    title: '6. 市场法',
    body: '流通权益 = POOL 倍数 × 基数；非流通 = 流通 × (1 − 市场法缺乏流动性折扣)，默认 30%，与并购 DCF 折扣分开。结果对比低端用 −1σ 倍数，高端用中位数，不取 +1σ。亏损年仍算 P/E，并告警「利润为负，仅供参考」。行业法开启时，用申万三级 PE/PS 中位数替换个股 POOL，同样按锚定日及以前历史中位。',
  },
  {
    id: 'dcf',
    title: '7. DCF',
    body: '预测期自由现金流按所选口径折现；折现期数按锚定日对齐，不因删掉更早的已实现年而把首年当成 t=1。终值二选一：退出 P/E × 末期净利润，或退出 P/S × 末期收入；工作台只启用对应那一格倍数，改另一格不会动 DCF。折现率默认填汇总 30%；也可填 WACC 分项（无风险利率、ERP、Beta，D/E 与债务成本可空），三项齐了才覆盖汇总折现率，不反算。单套净利润桥：股权价值 = 企业价值 − 净负债，不乘流动性折扣。选「并购 + 上市并排」时，市场法 P/S、P/E 只用市场法折扣；并购 DCF 股权价值再乘 (1−并购流动性折扣)，上市 DCF 不扣。折现率、退出倍数仍可按情景分填。敏感性默认退出 PE 20x～60x、收入 CAGR 以中位为中心 ±5%；结果对比取内圈四角，不是整张表最外的极端点。',
  },
  {
    id: 'result',
    title: '8. 结果对比',
