#!/bin/bash
# 从本地 dist 目录复制到 volume
# 在服务器上执行: cd /opt/newsapp/news && bash deploy/从本地dist复制到volume.sh

cd /opt/newsapp/news

echo "=========================================="
echo "从本地 dist 复制到 volume"
echo "=========================================="

echo ""
echo "步骤1: 检查本地 dist 目录..."
echo "----------------------------------------"
...
if [ -z "$FOUND_LOCATION" ]; then
    ...
    for loc in "${POSSIBLE_LOCATIONS[@]}"; do
        if [ -d "$loc" ]; then
            echo "  $loc 存在，但 index.html 不存在或太小"
            ls -la "$loc" | head -5
        else
            echo "  $loc 不存在"
        fi
    ...
    echo ""
    echo "请先上传构建文件到服务器，或确保构建文件在正确位置"
    exit 1
fi
